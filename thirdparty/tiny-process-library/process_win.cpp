...
Process::id_type Process::open(const std::string &command, const std::string &path) {
	...
	if (path == "")
		...
		strcpy_s(path_cstr, path.size() + 1, path.c_str());
	}
	...
	strcpy_s(command_cstr, command.size() + 1, command.c_str());
#endif

	size_t size = command.size() + 1;
	wchar_t* command_wchar = new wchar_t[size];
	size_t outSize;
	mbstowcs_s(&outSize, command_wchar, size, command_cstr, size - 1);
	LPWSTR command_lpwstr = command_wchar;

	size = path.size() + 1;
	wchar_t* path_wchar = new wchar_t[size];
	outSize;
	mbstowcs_s(&outSize, path_wchar, size, path_cstr, size - 1);
	LPWSTR path_lpwstr = path_wchar;

	BOOL bSuccess = CreateProcess(NULL, command_lpwstr, NULL, NULL, TRUE, 0,
		NULL, NULL/*path_lpwstr*/, &startup_info, &process_info);
	delete[] path_cstr;
	delete[] command_cstr;
	delete[] path_wchar;
	delete[] command_wchar;

	auto STUCKDRECK = GetLastError();
...
void Process::async_read() {
	...
	if (stdout_fd) {
		stdout_thread = std::thread([this]() {
			...
			for (;;) {
				BOOL bSuccess = ReadFile(*stdout_fd, buffer, static_cast<DWORD>(buffer_size), &n, NULL);
				if (!bSuccess || n == 0)
					break;
	...
	if (stderr_fd) {
		stderr_thread = std::thread([this]() {
			...
			char* buffer = new char[buffer_size];
			for (;;) {
				BOOL bSuccess = ReadFile(*stderr_fd, buffer, static_cast<DWORD>(buffer_size), &n, NULL);
				if (!bSuccess || n == 0)
					break;
